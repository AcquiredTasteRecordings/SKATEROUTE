        ...
        .sheet(isPresented: $isSearching) {
            PlaceSearchView(
                ...
                currentLocationProvider: {
                    AppDI.shared.locationManagerService.currentLocation?.coordinate
                },
                onPick: { place in
                    let dst = place.placemark.coordinate
                    let src = AppDI.shared.locationManagerService.currentLocation?.coordinate ?? dst
                    coordinator.router = .map(source: src, destination: dst, mode: .smoothest)
                    isSearching = false
                }
            )
        }

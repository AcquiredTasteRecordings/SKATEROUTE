...
public struct MapScreen: View {
    ...
    public var body: some View {
                    ...
                    Group {
                        if let r = route {
                            VStack(alignment: .trailing, spacing: 4) {
                                Text("Slope penalty: \(Int(slopePenalty * 100))%")
                                    .font(.footnote)
                                Text("Braking zones: \(brakingMask.filter { $0 }.count)")
                                    .font(.footnote)
                                Text("Steps: \(r.steps.count)")
                                    .font(.footnote)
                            }
                            .padding(8)
        ...
        .onReceive(locationService.$currentLocation) { optionalLoc in
            guard let loc = optionalLoc as? CLLocation else { return }
            lastLocation = loc
            ...
            if let r = route {
                ...
                let sample = MatchSample(location: loc, roughnessRMS: smoothness.currentRMS ?? 0.0)
                if let i = matcher.nearestStepIndex(on: r, to: sample) {
                    let stepId = SegmentStore.shared.makeStepId(route: r, stepIndex: i)
...
private extension MapScreen {
    ...
    func startRide() {
        ...
        AppDI.shared.motionService.start()
    ...
    func stopRide() {
        ...
        AppDI.shared.motionService.stop()

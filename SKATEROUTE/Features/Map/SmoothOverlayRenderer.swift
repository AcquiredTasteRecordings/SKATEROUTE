...
public enum SmoothOverlayBuilder {
    ...
    public static func build(route: MKRoute, colors: [UIColor], brakingMask: [Bool]) -> [ColoredPolyline] {
        ...
        for i in 0..<count {
            ...
            // Assign color for this segment or default color
            let currentColor = i < colors.count ? colors[i] : .systemBlue
            seg.color = currentColor
            
            // Assign nextColor for gradient transition if possible
            if i + 1 < colors.count {
                seg.nextColor = colors[i + 1]
            }
            ...
            if i < brakingMask.count, brakingMask[i] == true {

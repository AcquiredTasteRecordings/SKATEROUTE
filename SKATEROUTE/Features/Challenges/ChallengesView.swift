...
public struct ChallengeSummary: Identifiable, Equatable, Sendable {
    ...
    public init(id: String, title: String, kind: Kind, goalValue: Double, unitLabel: String, weekNumber: Int, startDate: Date, endDate: Date, isJoined: Bool, isPremiumGated: Bool) {
        self.id = id; self.title = title; self.kind = kind; self.goalValue = goalValue; self.unitLabel = unitLabel
        self.weekNumber = weekNumber; self.startDate = startDate; self.endDate = endDate
        self.isJoined = isJoined; self.isPremiumGated = isPremiumGated
    }
}
...
public struct ChallengeProgress: Equatable, Sendable {
    ...
    public init(challengeId: String, currentValue: Double, percent: Double, lastUpdated: Date) {
        self.challengeId = challengeId; self.currentValue = currentValue; self.percent = percent; self.lastUpdated = lastUpdated
    }
}
...
public struct StreakInfo: Equatable, Sendable {
    ...
    public init(currentStreakWeeks: Int, bestStreakWeeks: Int, onTrackThisWeek: Bool) {
        self.currentStreakWeeks = currentStreakWeeks; self.bestStreakWeeks = bestStreakWeeks; self.onTrackThisWeek = onTrackThisWeek
    }
}
...
public struct AnalyticsEvent: Sendable, Hashable {
    public enum Category: String, Sendable { case challenges }
...
public final class ChallengesViewModel: ObservableObject {
    ...
    @Published public private(set) var streak: StreakInfo = .init(currentStreakWeeks: 0, bestStreakWeeks: 0, onTrackThisWeek: false)
    ...
    public init(reader: ChallengeReading, actor: ChallengeActing, analytics: AnalyticsLogging?) {
        self.reader = reader
        self.actor = actor
    ...
    private func applySummaries(_ list: [ChallengeSummary]) {
        summaries = list.sorted { a, b in
            // Active week first by startDate; then distance/cleanup preference
        ...
        // Cancel obsolete
        for (id, c) in progressCancellables where !ids.contains(id) { c.cancel(); progressCancellables.removeValue(forKey: id) }
        // Subscribe new
        for id in ids where progressCancellables[id] == nil {
            let c = reader.progressPublisher(for: id)
                .receive(on: RunLoop.main)
                .sink { [weak self] p in self?.progress[id] = p }
    ...
    public func join(_ ch: ChallengeSummary) {
        ...
        Task {
            do {
                ...
                analytics?.log(.init(name: "challenge_join", category: .challenges, params: ["id": .string(ch.id)]))
    ...
    public func leave(_ ch: ChallengeSummary) {
        ...
        Task {
            do {
                ...
                analytics?.log(.init(name: "challenge_leave", category: .challenges, params: ["id": .string(ch.id)]))
    ...
    private func optimisticToggle(_ ch: ChallengeSummary, toJoin: Bool) {
        if let idx = summaries.firstIndex(where: { $0.id == ch.id }) {
            var m = summaries[idx]
            m = .init(id: m.id, title: m.title, kind: m.kind, goalValue: m.goalValue, unitLabel: m.unitLabel,
                      weekNumber: m.weekNumber, startDate: m.startDate, endDate: m.endDate,
                      isJoined: toJoin, isPremiumGated: m.isPremiumGated)
            summaries[idx] = m
        }
    }
    ...
    private func rollbackToggle(_ ch: ChallengeSummary) {
        if let idx = summaries.firstIndex(where: { $0.id == ch.id }) {
            ...
            m = .init(id: m.id, title: m.title, kind: m.kind, goalValue: m.goalValue, unitLabel: m.unitLabel,
                      weekNumber: m.weekNumber, startDate: m.startDate, endDate: m.endDate,
                      isJoined: !m.isJoined, isPremiumGated: m.isPremiumGated)
            summaries[idx] = m
        }
    }
...
public struct ChallengesView: View {
    ...
    public init(viewModel: ChallengesViewModel) { self.vm = viewModel }
    ...
    public var body: some View {
        ...
        .sheet(item: $vm.showingRulesFor, onDismiss: { rulesText = "" }) { ch in
            RulesSheet(title: ch.title,
                       load: {
                           if rulesText.isEmpty {
                               loadingRules = true
                               Task { rulesText = await vm.rulesText(for: ch); loadingRules = false }
                           }
                       },
                       text: $rulesText,
    ...
    private var header: some View {
        HStack(spacing: 8) {
            Image(systemName: "flag.checkered").imageScale(.medium)
    ...
    private var streakCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                StreakPill(title: NSLocalizedString("Current", comment: "cur"), value: vm.streak.currentStreakWeeks)
                StreakPill(title: NSLocalizedString("Best", comment: "best"), value: vm.streak.bestStreakWeeks)
            }
            .accessibilityElement(children: .combine)
            .accessibilityLabel(Text("\(vm.streak.currentStreakWeeks) week current streak. Best \(vm.streak.bestStreakWeeks) weeks."))
        }
        .padding(12)
...
fileprivate struct ChallengeRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(rowTitle).font(.subheadline.weight(.semibold))
                    Text(weekLabel).font(.caption).foregroundStyle(.secondary)
                }
                Spacer()
                if ch.isPremiumGated {
                    Label(NSLocalizedString("Premium", comment: "premium"), systemImage: "star.fill")
                        .font(.caption2.weight(.semibold))
                        .padding(.horizontal, 8).padding(.vertical, 4)
                        .background(Color.yellow.opacity(0.2), in: Capsule())
                        .accessibilityLabel(Text(NSLocalizedString("Premium challenge", comment: "")))
                }
            }
            ...
            HStack(spacing: 8) {
                if ch.isJoined {
                    Button(action: leave) { Label(NSLocalizedString("Leave", comment: "leave"), systemImage: "person.crop.circle.badge.minus") }
                        .buttonStyle(.bordered)
                } else {
                    Button(action: join) { Label(NSLocalizedString("Join", comment: "join"), systemImage: "person.crop.circle.badge.plus") }
                        .buttonStyle(.borderedProminent)
                }
                Button(action: rules) { Label(NSLocalizedString("Rules", comment: "rules"), systemImage: "info.circle") }
                    .buttonStyle(.bordered)
                Spacer()
            }
            .frame(minHeight: 44)
        ...
        .padding(.vertical, 6)
        .accessibilityElement(children: .combine)
        .accessibilityLabel(Text("\(rowTitle). \(progressAX)."))
    }
    ...
    private var weekLabel: String {
        ...
        return String(format: NSLocalizedString("Week %d • %@–%@", comment: "week"),
                      ch.weekNumber, fmt.string(from: ch.startDate), fmt.string(from: ch.endDate.addingTimeInterval(-86400)))
    }
    ...
    private var progressAX: String {
        ...
        return String(format: NSLocalizedString("%d percent complete", comment: "ax percent"), percent)
    }
}
        ...
        NavigationView {
            Group {
                if loading {
                    ...
                    ScrollView {
                        Text(text.isEmpty ? NSLocalizedString("Rules unavailable.", comment: "no rules") : text)
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(NSLocalizedString("Close", comment: "close")) { UIApplication.shared.sendAction(#selector(UIResponder.resignFirstResponder), to: nil, from: nil, for: nil) }
                }
            }
...
public extension ChallengesView {
    static func make(reader: ChallengeReading, actor: ChallengeActing, analytics: AnalyticsLogging? = nil) -> ChallengesView {
...
final class ReaderFake: ChallengeReading {
    ...
    private let streakSubj = CurrentValueSubject<StreakInfo, Never>(.init(currentStreakWeeks: 3, bestStreakWeeks: 7, onTrackThisWeek: true))
    func progressPublisher(for challengeId: String) -> AnyPublisher<ChallengeProgress, Never> {
        let start = Double(Int.random(in: 0...5000))
        let subj = CurrentValueSubject<ChallengeProgress, Never>(.init(challengeId: challengeId, currentValue: start, percent: min(start/10000.0, 1.0), lastUpdated: Date()))
        // Simulate ticking progress
        Timer.publish(every: 2.0, on: .main, in: .common).autoconnect()
            .sink { _ in
                let nextVal = min((subj.value.currentValue + Double(Int.random(in: 120...680))), 10000)
                subj.send(.init(challengeId: challengeId, currentValue: nextVal, percent: nextVal/10000.0, lastUpdated: Date()))
            }.store(in: &cancellables)
        return subj.eraseToAnyPublisher()
    }
    var summariesPublisher: AnyPublisher<[ChallengeSummary], Never> { summariesSubj.eraseToAnyPublisher() }
    var streakPublisher: AnyPublisher<StreakInfo, Never> { streakSubj.eraseToAnyPublisher() }
    private var cancellables = Set<AnyCancellable>()
    init(now: Date = Date()) {
        ...
        summariesSubj.send([
            .init(id: "W\(week)-distance", title: "Weekly Distance", kind: .distance, goalValue: 10_000, unitLabel: "km", weekNumber: week, startDate: start, endDate: end, isJoined: true, isPremiumGated: false),
            .init(id: "W\(week)-cleanup", title: "Hazard Clean-up", kind: .cleanup, goalValue: 5, unitLabel: NSLocalizedString("hazards", comment: "hazards"), weekNumber: week, startDate: start, endDate: end, isJoined: false, isPremiumGated: false)
        ])
    }
}
...
struct ChallengesView_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView { ChallengesView.make(reader: ReaderFake(), actor: ActorFake()) }
...
// • Analytics: log join/leave, rules open; never log precise distance traces.

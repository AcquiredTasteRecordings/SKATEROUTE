...
public struct GeocoderService {
    ...
    public func forward(query: String, near region: MKCoordinateRegion? = nil, useAppleMapsPriority: Bool = false) async throws -> [MKMapItem] {
        ...
        let task = Task<[MKMapItem], Error> {
            ...
            // Bias toward Apple-verified POIs by limiting result types to points of interest (stable across iOS versions)
            ...
            do {
                ...
                if items.isEmpty {
                    throw GeocoderServiceError.noResultsFoundQuery(query)
                }
    ...
    public func reverse(location: CLLocationCoordinate2D) async throws -> [MKMapItem] {
        ...
        do {
            ...
            if placemarks.isEmpty {
                throw GeocoderServiceError.noResultsFoundLocation(location)
            }
    ...
    public enum GeocoderServiceError: LocalizedError {
        case noResultsFoundQuery(_ query: String)
        case noResultsFoundLocation(_ location: CLLocationCoordinate2D)
        case forwardGeocodingFailed(underlyingError: Error)
        case reverseGeocodingFailed(underlyingError: Error)

        public var errorDescription: String? {
            switch self {
            case .noResultsFoundQuery(let query):
                return "No results found for query: \"\(query)\"."
            case .noResultsFoundLocation(let location):

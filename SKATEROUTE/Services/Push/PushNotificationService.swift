...
public protocol PushRemoteAPI {
  ...
  func registerToken(_ token: String, topics: Set<String>, locale: String, appVersion: String)
    async throws
...
public final class PushNotificationService: NSObject, ObservableObject {
  ...
  public func emitBanner(
    title: String, body: String, category: String? = nil, userInfo: [AnyHashable: Any] = [:]
  ) {
    bannerSubject.send(
      Banner(title: title, body: body, category: category, userInfo: userInfo, receivedAt: Date()))
  }
  ...
  private func loadPersisted() {
    ...
    if let arr = defaults.array(forKey: Self.udkTopics) as? [String] {
      topics = Set(
        arr.map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty })
    }
  }
  ...
  private func sanitizedTopics(_ inSet: Set<String>) -> Set<String> {
    ...
    Set(
      inSet.compactMap { s in
        let norm = s.lowercased().replacingOccurrences(
  ...
  private func registerCategoriesFromPlist() {
    guard let url = Bundle.main.url(forResource: "NotificationCategories", withExtension: "plist"),
      let arr = NSArray(contentsOf: url) as? [[String: Any]]
    else {
      return
    }
    ...
    for dict in arr {
      ...
      let actions: [UNNotificationAction] = actionsArr.compactMap { a in
        guard let aid = a["identifier"] as? String, let title = a["title"] as? String else {
          return nil
        }
      ...
      let cat = UNNotificationCategory(
        identifier: id, actions: actions, intentIdentifiers: [], options: opts)
  ...
  private static func categoryOptions(_ arr: [String]) -> UNNotificationCategoryOptions {
    ...
    for s in arr {
      ...
      case "hiddenpreviewshowtitle":
        if #available(iOS 11.0, *) { o.insert(.hiddenPreviewsShowTitle) }
      case "hiddenpreviewshowsubtitle":
        if #available(iOS 11.0, *) { o.insert(.hiddenPreviewsShowSubtitle) }
      default: break
      }
    }
    return o
...
extension PushNotificationService: UNUserNotificationCenterDelegate {
  ...
  public func userNotificationCenter(
    ...
    let isSafety =
      content.categoryIdentifier == "HAZARD_NEARBY" || content.categoryIdentifier == "RIDE_ALERT"
...
// MARK: - DEBUG fakes (for tests)
  ...
  public final class PushAPIFake: PushRemoteAPI {
    public private(set) var registered:
      (token: String, topics: Set<String>, locale: String, appVersion: String)?
    public private(set) var updated: (token: String, topics: Set<String>)?
    public init() {}
    public func registerToken(
      _ token: String, topics: Set<String>, locale: String, appVersion: String
    ) async throws {
